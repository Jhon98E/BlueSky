...
export class VueloModel {
  ...
  static async crear({ origen, destino, fecha_salida, fecha_regreso, precio, tipo_vuelo }) {
  ...
  static async actualizar({ id, origen, destino, fecha_salida, fecha_regreso, precio, tipo_vuelo }) {    
    const [result] = await db.query(
  ...
  static async eliminar({ id }) {
    const [result] = await db.query(
      `DELETE FROM Vuelo WHERE vuelo_id =?;`,
      [id]
    )
    return result.affectedRows
  }
